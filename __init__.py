...
class MilightSkill(OnyxSkill):
    ...
    def handle_light_off(self, message):
        light_zone = message.data.get('LightZone')
        try:
            self.controller.send(self.light.off(light_zone))
            self.speak_dialog("off.one")
        except Exception as e:
            LOGGER.error("Error: {0}".format(e))
    ...
    def handle_light_color(self, message):
        ...
        light_zone = message.data.get("LigtZone")
        try:
    ...
    def handle_light_brightness(self, message):
        brightness_level = message.data.get("Brightness")
        light_zone = message.data.get("LightZone")
        if brightness not in range (0,101):
            self.speak_dialog("the.light.brightness.must.be.between.one.and.one.hundred")
        else:
